...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800"><?= esc($subtitle ?? 'Daftar Pengajuan Kuota') ?></h1>
    </div>
                        ...
                        <?php if (!empty($pengajuan_kuota) && is_array($pengajuan_kuota)) : ?>
                            ...
                            <?php foreach ($pengajuan_kuota as $pk) : ?>
                                <tr>
                                    ...
                                    <td>
                                        <a href="<?= base_url('petugas_administrasi/detailPengajuanKuotaAdmin/' . ($pk['id'] ?? '')) ?>" class="btn btn-info btn-circle btn-sm my-1" title="Lihat Detail Proses Admin">
...
$(document).ready(function() {
    // Pastikan jQuery dan DataTables sudah dimuat di template layout utama Anda
    if (typeof $ !== 'undefined' && typeof $.fn.DataTable !== 'undefined') {
        $('#dataTableAdminPengajuanKuota').DataTable({
            "order": [[ 8, "desc" ]], // Urutkan berdasarkan Tgl Submit Sistem terbaru
            "language": {
            ...
            "columnDefs": [
                { "orderable": false, "targets": [0, 13] } // Non-sortable columns
            ]
        });
    } else {
